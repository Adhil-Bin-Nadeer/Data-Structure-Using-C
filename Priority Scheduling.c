...
int main() {
        ...
        {
                printf(" Enter the priority of P%d : ", sjf[i].Process_Id);
                scanf("%d", & sjf[i].Priority);
        }

        //Sorting the array using bubble sort
        for (int i = 0; i < processes - 1; i++) {
                for (int j = 0; j < processes - i - 1; j++) {
                        if (sjf[j + 1].Priority < sjf[j].Priority) {
                                ...
                                temp = sjf[j + 1].Priority;
                                sjf[j + 1].Priority = sjf[j].Priority;
                                sjf[j].Priority = temp;
                        }
                }
        }
        ...
        printf("\n\nProcess ID\tBurst-Time\tPriority-Time\tTurn-Around-Time\tWaiting-Time");
        for (int i = 0; i < processes; i++) {
                printf("\n P%d\t\t %d\t \t%d\t\t %d\t\t\t%d", sjf[i].Process_Id,
                        sjf[i].Burst_Time, sjf[i].Priority, sjf[i].Completion_Time, sjf[i].Waiting_Time);
        }
